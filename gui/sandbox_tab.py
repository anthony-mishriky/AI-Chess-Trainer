...
class SandboxTab(ctk.CTkFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        self.setup_mode = True # True = Editing, False = Sparring
    ...
    def setup_left_panel(self):
        ...
        ctk.CTkButton(btn_frame, text="Load FEN", width=90, fg_color="#333333", hover_color="#444444", command=self.load_fen).pack(side="left", padx=5)
        ctk.CTkButton(btn_frame, text="Copy FEN", width=90, fg_color="#333333", hover_color="#444444", command=self.copy_fen).pack(side="left", padx=5)
    ...
    def setup_right_panel(self):
        ...
        ctk.CTkButton(btn_row, text="Starting Pos", width=90, fg_color="#3a7ebf", hover_color="#1f538d", command=self.set_starting_pos).pack(side="left", padx=5)
        ctk.CTkButton(btn_row, text="Clear Board", width=90, fg_color="#8B0000", hover_color="#5C0000", command=self.clear_board).pack(side="left", padx=5)
    ...
    def apply_ai_move(self, move):
        if move and not self.setup_mode:
            ...
            if self.board.is_game_over():
                ...
                try: pygame.mixer.Sound("assets/move.wav").play()
                except: pass
            self.draw_board()

    def update_history(self):
        self.history_box.delete("0.0", "end")
        temp_board = chess.Board(self.fen_entry.get()) # Start from initial FEN
        history_text = ""
        ...
        for row in range(8):
            for col in range(8):
                ...
                if not self.setup_mode:
                    ...
                    if self.selected_square == square_idx: color = "#D2691E" 
        ...
        if self.selected_square is not None and not self.setup_mode:
            for move in self.board.legal_moves:
                if move.from_square == self.selected_square:
                    ...
                    if self.board.piece_at(move.to_square): self.canvas.create_oval(cx - 38, cy - 38, cx + 38, cy + 38, outline="#333333", width=5)
                    else: self.canvas.create_oval(cx - 12, cy - 12, cx + 12, cy + 12, fill="#333333", outline="")
            ...
            if square == self.selected_square: self.selected_square = None
                ...
                if moving_piece and moving_piece.piece_type == chess.PAWN and (square // 8 == 0 or square // 8 == 7):
                    if chess.Move(self.selected_square, square, promotion=chess.QUEEN) in self.board.legal_moves:
                        move = chess.Move(self.selected_square, square, promotion=chess.QUEEN) # Sandbox defaults to Queen for speed
    ...
    def continuous_eval(self):
        ...
        while True:
            time.sleep(0.5)
            if self.setup_mode and self.eval_engine and self.board.is_valid() and self.board.fen() != last_fen:
                try:
                    ...
                    bar_fill = max(0.0, min(1.0, (val + 5.0) / 10.0))
                    self.after(0, lambda v=val, f=bar_fill: self.update_eval_ui(v, f))
                except Exception: pass
    ...
    def show_warning_modal(self, title, message):
        ...
        ctk.CTkLabel(dialog, text=message, font=("Arial", 14)).pack(pady=(0, 20))
